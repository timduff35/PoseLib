...
// Adjugate of a general 3x3 matrix (also defined for singular matrices).
inline Eigen::Matrix3d adjugate(const Eigen::Matrix3d &H) {
    Eigen::Matrix3d adj;
...
// An orthonormal basis of the plane perpendicular to n.
inline Eigen::Matrix<double, 3, 2> perpendicular(const Eigen::Vector3d &n) {
    Eigen::Matrix<double, 3, 2> basis;
    basis.col(0) = n.unitOrthogonal();
    basis.col(1) = n.normalized().cross(basis.col(0));
    return basis;
}
